...
=head1 NAME
  ...
  train_jnet.pl - wrapper script to train bagging version of Jnet
...
my $QSUB     = 'qsub -q 64bit.q -l qname=64bit.q -m a -M a.sherstnev@dundee.ac.uk -R y -l ram=4000M -cwd';
my $QSUB_ACC = 'qsub -q c6145.q -l qname=c6145.q -m a -M a.sherstnev@dundee.ac.uk -R y -l ram=4000M -cwd';
...
GetOptions(
  ...
  'cross-val=i' => \$nfolds,
  'wdir=s'      => \$rootDir,
  'bagging=s'   => \$bagging_fraction,
  'nnets=s'     => \$bagging_nnets,
  'hard!'       => \$hard,
...
pod2usage( -msg => "The bagging fraction can be between 0.0 and 1.0\n",        -verbose => 0 ) if ( 0.0 > $bagging_fraction or 1.0 < $bagging_fraction );
pod2usage( -msg => "The number of bagging networks should be from 2 to 50.\n", -verbose => 0 ) if ( 0 > $bagging_nnets or 50 < $bagging_nnets );
...
while ( $set <= $nfolds ) {
  ...
  if ($whole) {
    if ( $bagging_fraction < 1.0 ) {
      ...
      $workDir = "$rootDir/bagging-net$set";
      $dataDir = "$workDir/raw_data";
      my $dataDir_tmp = "$workDir/data";
      my $valid       = "$workDir/valid";
      mkdir($workDir)                                                or die "ERROR! unable to mkdir for '$workDir' (working directory): $!";
      mkdir($dataDir)                                                or die "ERROR! unable to mkdir for '$dataDir' (raw data directory): $!";
      mkdir($dataDir_tmp)                                            or die "ERROR! unable to mkdir for '$dataDir_tmp' (data directory): $!";
      mkdir($valid)                                                  or die "ERROR! unable to mkdir for '$valid' (validation data): $!";
      ...
      my $ntotaldomains = $#files + 1;
      my $ntraindomains = int( $ntotaldomains * $bagging_fraction );
      shuffle( \@files );
      my $i = 0;
      ...
      foreach my $file (@files) {
        ...
        my @store = map { "$dataDir/$id.$_" } qw(hmm pssm);
        if ( $i < $ntraindomains ) {
          system("mv @store $dataDir_tmp") == 0 or die "ERROR! unable to move validation set for $id";
        } else {
          system("mv @store $valid") == 0 or die "ERROR! unable to move validation set for $id";
        }
        ++$i;
      }
      $dataDir = $dataDir_tmp;
...
sub scoring {
  ...
  if ( $whole and $bagging_fraction >= 1.0 ) {
    $dataDir  = $DATA;
    $validDir = $VALID;
  }

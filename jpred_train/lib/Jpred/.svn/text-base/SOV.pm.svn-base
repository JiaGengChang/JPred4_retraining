...
sub new {
  my ( $class, %args ) = @_;
  my $self = {
    path   => '/homes/chris/projects/Jnet/sov',
    output => 'sov.out',
    input  => 'sov.in'
  };
  bless( $self, $class );
...
### Method to run the SOV calulation
## The executable and input path values are tested at this before running SOV.
## Checks that the output is created.
sub run {
  my $self = shift;

  die "ERROR - SOV input '$self->{input}' not found"        unless ( -e $self->{input} );
  die "ERROR - sov executable not found at '$self->{path}'" unless ( -e "$self->{path}/sov" );
  die "ERROR - sov not executable at '$self->{path}'"       unless ( -x "$self->{path}/sov" );

  my $cmd = "$self->{path}/sov $self->{input} > $self->{output}";

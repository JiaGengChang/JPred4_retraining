...
our @ISA    = qw(Exporter);
our @EXPORT = qw(connect_DB get_DSSP check_DB get_seqID get_dataset db_path get_seq get_accDB);
...
sub connect_DB {
  ...
  $dbh = DBI->connect( "dbi:mysql:jnet:$server", $user, 'funguy33', { AutoCommit => 1 } ) or die "ERROR! can't connect to db: ", $DBI::errstr;
  unless ($dbh) {
    return (0);
  }
  return ($dbh);

...
sub train_net ($) {
  ...
  system("/sw/bin/batchman -f $root.bat") == 0 or die "ERROR - failed to run batchman";
  if ( -e "$root.net" ) {
    system("/sw/bin/snns2c $root.net") == 0 or die "ERROR - failed to run snns2c";
  } else {
    die "ERROR - '$root.net' network file not found. Check that batchman ran correctly.";
  }
  if ( -e "$root.c" ) {
    system("gcc -c $root.c") == 0 or die "ERROR - failed to compile '$root.c'";
  } else {
  ...
  if ( -e "${root}net.c" ) {
    system("gcc -Wall -o ${root}net ${root}net.c $root.o -lm") == 0 or die "ERROR - failed to compile '${root}net.c'.";
  } else {

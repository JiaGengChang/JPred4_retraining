...
my $QSUB     = 'qsub -pe smp 8 -m a -M 2472402@dundee.ac.uk -R y -mods l_hard mfree 16G -jc long -cwd';      # need -jc long otherwise hmm will time out (jiageng)
my $QSUB_ACC = 'qsub -pe smp 4 -m a -M 2472402@dundee.ac.uk -R y -mods l_hard mfree 4G -jc short -cwd';
...
my $SRCROOT = '/cluster/gjb_lab/2472402/jpred_train';                                                        # path to root directory to all Jpred codes
my $JNETSRC = $SRCROOT.'/jnet_src_v.2.3.1';                                                                  # source path for Jnet code
my $SCRIPTS = '/cluster/gjb_lab/2472402/jpred_train';                                                        # path to scripts required for training
...
while ( $set <= $nfolds ) {
  ...
  if ($whole) {
    ...
    system("find $DATA -type f -maxdepth 1 -exec ln -s {} $dataDir \\;") == 0 or die "ERROR! unable to copy datafiles"; # Jia Geng. added new option -maxdepth 1
    ...
    foreach my $file (@validSet) {
      ...
      system("mv @store $valid") == 0 or die "ERROR! unable to move validation set for $root. File is $file,\n";
  ...
  foreach my $profile (qw(pssm hmm)) {
    ...
    if ( $profile =~ /pssm/ ) {
      ...
      for my $cut (qw(0 5 25)) {
        ...
        print "command: $cmd\n" if ($DEBUG);
        my $out = ` $QSUB_ACC $cmd` or die "ERROR! qsub failed for $name";
        my $id = ( split( /\s+/, $out ) )[2];
        if ( $id !~ /^\d+$/ ) {
...
sub scoring {
  ...
  if ($validDir) {
    ...
    system("make CC=gcc -C $currJnet --silent 2>/dev/null");
    die "ERROR! Jnet failed to compile!\nDied" unless ( -e "$currJnet/jnet" );
    $cmd = "$SCRIPTS/score_jnet.pl  -data $validDir -jnet $currJnet -jnet-version 2.1 -pred-items jnetpred,jnetpssm,jnethmm -out score_jnetval.csv -delete";
    system("unset LD_GOLD");

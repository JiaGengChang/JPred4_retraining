...
paths = ["/homes/adrozdetskiy/Projects/JnetDatasets/Jnet_training_output_v2/cross-val%d/" % num for num in range(6,7)]
...
for counter, path in enumerate(paths):
    ...
    train_files = [f for f in os.listdir(train_path) if f[-5:] == '.pssm'] # change to f[-4:] for HMM
    valid_files = [f for f in os.listdir(valid_path) if f[-5:] == '.pssm'] # change to f[-4:] for HMM

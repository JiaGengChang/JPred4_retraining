#!/cluster/gjb_lab/2472402/ml-env/bin/python3.9
#$ -pe smp 16
#$ -jc short
#$ -adds l_hard gpu 4
...
#$ -wd /cluster/gjb_lab/2472402/outputs/2021-09-16 
...
# to silence verbosity from tensorflow
...
subtype='PSSM' # <-- change this value
...
for i in range(1,8):
    ...
    root_folder="26Aug_w_callback"
    #root_folder="/cluster/gjb_lab/2472402/outputs/keras_train_CV/%s/26Aug_tb_2" % subtype # output directory. remember to change
    if not path.exists(root_folder):
        os.system("mkdir %s" % root_folder)
    # create out folder
    out_folder=os.path.join(root_folder, "%s-cross-val%d/" % (subtype,i))
    ...
    tensorboard_callback = keras.callbacks.TensorBoard(
                log_dir=os.path.join(out_folder,"layer1"), 
                histogram_freq=10,
                )
    ...
    tensorboard_callback_2 =  keras.callbacks.TensorBoard(
                log_dir=os.path.join(out_folder,"layer2"), 
                histogram_freq=10,
                )
